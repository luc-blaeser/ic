...
unsafe impl wasmtime::MemoryCreator for WasmtimeMemoryCreator {
    fn new_memory(
        ...
        let wasm64_max_memory_in_bytes: u64 =
            core::cmp::max(MAX_STABLE_MEMORY_IN_BYTES, MAX_WASM_MEMORY_IN_BYTES);

        let max_pages = if ty.is_64() {
            wasm64_max_memory_in_bytes / (WASM_PAGE_SIZE as u64)
        } else {
            WASM32_MAX_PAGES
        };
        ...
        let reserved_size = reserved_size_in_bytes.unwrap_or(max * WASM_PAGE_SIZE as usize);

        assert!(
            reserved_size <= wasm64_max_memory_in_bytes as usize,
            "Reserved bytes for wasm memory {} exceeds the maximum expected {}",
            reserved_size,
            wasm64_max_memory_in_bytes
        );
...
impl MmapMemory {
    pub fn new(mem_size_in_bytes: usize, guard_size_in_bytes: usize) -> Self {
        let guard_size_in_bytes = core::cmp::min(guard_size_in_bytes, MIN_GUARD_REGION_SIZE);
        assert!(
